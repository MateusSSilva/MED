...
def MED(*args, trim_border=True):
    ...
    for i in range(nDim):
        ...
        values = segment_MED(outputVar.loc[dim[i], "timeSeries"]["t"],
                             ...
                             minD, minT, minV)
        outputVar.at[dim[i], "ME"] = values if values.size != 0 else None
    ...
    def safe_concat(data_list):
        ...
        return np.concatenate(valid_items).tolist() if valid_items else []
...
def segment_MED(t, r, v, min_D, min_T, min_V):
    ...
    if pk_class[-1] == 1: seg_i = seg_i[:-1]
    if pk_class[0]  == 1: seg_f = seg_f[1:]
...
def analyze_elements_MED(t, r, v, ME):
    ...
    """
    N  = ME.shape[0]
    Nt = N / (t[-1] - t[0]) if t[-1] != t[0] else 0
    ...
    for i in range(N):
        ...
        W[i]  = np.std(Hoff - v_i, ddof=1) / V[i] if V[i] != 0 else 0
        corr  = np.corrcoef(Hoff, v_i)[0, 1] if len(v_i) > 1 else 0
        R2[i] = corr**2 if not np.isnan(corr) else 0
...
def scaling_MED(D_dict, V_dict, plot_name=None):
    ...
    for i, dim in enumerate(D_dict.keys()):
        ...
        alpha[dim]   = p[0]
        K[dim]       = np.exp(p[1])
        corr         = np.corrcoef(logD, logV)[0, 1]
        R2_vals[dim] = corr**2
        ...
        if plot_name:
            ...
            plt.loglog(D_dict[dim], V_dict[dim], 'o')
            plt.loglog(np.exp(x_fit), np.exp(y_fit), '-', linewidth=1.5)
            plt.title(dim)
            plt.xlabel("Displacement (m)")
            plt.ylabel("Average Velocity (m/s)")

    if plot_name:
        plt.suptitle("Scaling of Average Velocity (m/s) vs Displacement (m)")
