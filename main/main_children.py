...
Dataset
-------
Folder:    data/CP child gait data/td/ (searched recursively for *.csv)
Structure: filenames follow the pattern TD<subject><trial>.csv
           (e.g., TD12a.csv); first column is time [s], remaining columns
           are positions in millimetres; sampling rate derived from the
           time column.
...
Output
------
output/children/output_children_python.csv — one row per trial, columns:
file, ind, task, trial, then D/V/T/N/Nt/W/R2/P and alpha/K/R2_alpha for
each dimension (suffix _all, _x, _y, _z).
"""
...
data_dir   = os.path.join(repo_root, 'data', 'CP child gait data', 'td')
output_dir = os.path.join(repo_root, 'output', 'children')
...
for i, file_path in enumerate(files):
    ...
    core_name   = name_no_ext.replace("TD", "")

    if core_name[-1].isalpha():
        subject = core_name[:-1]
        trial   = core_name[-1]

    task = "walk"

    print(f"Processing: {file_name} | Subject: {subject} | Trial: {trial}")
...
tab.to_csv(os.path.join(output_dir, 'output_children_python.csv'), index=False)
